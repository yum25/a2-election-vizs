...
const draw = async (
  ...
  let dude = electionData[`${lineType} - Duderstadt Center/Pierpont`].map(
    (d) => ({
  ...
  let umma = electionData[
    `${lineType} - University of Michigan Museum of Art`
  ].map((d) => ({
    time: parseTime(d.time),
  ...
  if (umma_last_updated) {
    umma_last_updated = parseTime(
      `${date}, 2024 ${umma_last_updated.slice(
        0,
        5
      )}:00 ${umma_last_updated.slice(-2)}`
    );
  }
  ...
  if (dude_last_updated) {
    dude_last_updated = parseTime(
      `${date}, 2024 ${dude_last_updated.slice(
        0,
        5
      )}:00 ${dude_last_updated.slice(-2)}`
    );
  }
  ...
  div
    ...
    .html(
      `
			<p style="margin: 5px 0;"><b>${date}</b></p>
  ...
  svg
    ...
    .on("mouseover", function (e, d) {
      const posx =
        e.pageX > (width + margin.right + margin.left) / 2
          ? e.pageX - 230
          : e.pageX;
      const posy = e.pageY;
        ...
        .html(
          `<b>UMMA - ${lineType} Line</b>: <br/>${d.wait_time} minutes<br/>${
            d.line_length
					...
					<br/><i>${d.time.toLocaleTimeString([], {
            hour: "2-digit",
            minute: "2-digit",
          })}</i>`
        )
  ...
  svg
    ...
    .on("mouseover", function (e, d) {
      ...
      d3.select("#tooltip-times")
        .html(
          `<b>Duderstadt - ${lineType} Line</b>: <br/>${
            d.wait_time
          } minutes<br/>${d.line_length} people waiting
					<br/><i>${d.time.toLocaleTimeString([], {
            hour: "2-digit",
            minute: "2-digit",
          })}</i>`
...
const drawMap = async (i, yes, no) => {
  ...
  const getProportion = (feature, proposition) => {
    ...
    const filteredAA = proposition.report.data.filter(
      (d) => d.Precinct === ward
    )[0];
  ...
  // step 3: draw canvas
  const projection = d3.geoMercator().fitSize([mapWidth, mapHeight], annarbor);
  ...
  g.selectAll("path")
    ...
    .attr("fill-opacity", "1")
    .attr("fill", (d) =>
      getColor(getProportion(d, electionData.data[i]), d, electionData.data[i])
    )
    .attr("stroke", "white")
    .attr("stroke-width", 1)
